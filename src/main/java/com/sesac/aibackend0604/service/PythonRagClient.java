...
import com.sesac.aibackend0604.dto.ChatRequest;
    ...
    public Mono<RagIngestResponse> ingest(byte[] bytes, String filename) {
        ...
        return pythonWebClient.post()
                ...
                .onStatus(HttpStatusCode::is4xxClientError, resp -> {
                    ...
                    return Mono.error(new IllegalArgumentException("invalied file for python ingest"));
                })
                .onStatus(HttpStatusCode::is5xxServerError, resp -> {
                    log.warn("python ingest 5xx: {}", resp.statusCode());
